 ...
 */
trait PrismicController extends Controller {
  ...
  def I18nAction(code: String)(block: PrismicHelper.I18nRequest[AnyContent] => Future[Result]): Action[AnyContent] =
    i18nBodyAction(code, parse.anyContent)(block)

  // -- Action builder
  def i18nBodyAction[A](code: String, bodyParser: BodyParser[A])(block: PrismicHelper.I18nRequest[A] => Future[Result]) =
    Action.async(bodyParser) { implicit request =>
      for {
        ctx <- PrismicHelper.buildContext
        result <- block(PrismicHelper.I18nRequest(request, I18n.lang(code), ctx))
      } yield result
    }
}
